...
function getCurrentSet() {
  // Return first active set (or builtin if none active)
  const firstActiveId = activePatternSetIds.size > 0 
    ? Array.from(activePatternSetIds)[0] 
    : "builtin";
  return (
    patternSets.find((s) => s.id === firstActiveId) ||
...
function getImageSrc(item) {
  // Return URL if available, otherwise return null (no fallback folder)
...
function findAllImagesForChar(char) {
  ...
  patternSets.forEach((set) => {
    // Only search in active sets (all patterns in list are used)
    if (!activePatternSetIds.has(set.id)) return;
...
function getSetForRotation(position) {
  ...
  // Filter to only include active sets, preserving the order from patternSets
  const activeOrder = [];
  // Iterate through patternSets in order and add active ones
  for (const set of patternSets) {
    if (activePatternSetIds.has(set.id)) {
      activeOrder.push(set.id);
    }
  }
  ...
  const setIndex = position % activeOrder.length;
  const setId = activeOrder[setIndex];
