...
async function uploadImage(file, folderName = "test") {
  ...
  try {
    ...
    const res = await fetch(
      `https://qr.stability.ltd/upfileocean?fileName=${fileName}&folder=${folder}`,
              ...
              ${
                item.success
                  ? `<input type="text" value="${item.url}" readonly style="width: 100%; padding: 6px; border: 1px solid #bfdbfe; border-radius: 6px; font-size: 12px; background: white;" />
                     <button class="copy-single-url" data-url="${item.url}" style="margin-top: 5px; padding: 4px 8px; background: #667eea; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 11px;">📋 Copy</button>`
